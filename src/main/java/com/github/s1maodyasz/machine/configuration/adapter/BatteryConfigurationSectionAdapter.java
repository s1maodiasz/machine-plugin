...
import com.github.s1maodyasz.machine.configuration.BatteryConfiguration;
...
public final class BatteryConfigurationSectionAdapter {
    ...
    public static BatteryConfiguration adapt(ConfigurationSection section) {
        ...
        if (itemSection == null)
            throw new IllegalArgumentException("Battery configuration requires 'item' section.");

        final ItemConfiguration item = ItemConfigurationSectionAdapter.adapt(itemSection);
        if (item == null)
            throw new IllegalArgumentException("Battery configuration item could not be adapted.");
