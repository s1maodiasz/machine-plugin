...
public final class BatteryConfigurationSectionAdapter {
    ...
    public static BatteryConfiguration adapt(ConfigurationSection section) {
        ...
        final var itemSection = Objects.requireNonNull(
                section.getConfigurationSection("item"), "Battery configuration requires 'item' section.");
        final var item = Objects.requireNonNull(
                ItemConfigurationSectionAdapter.adapt(itemSection), "Battery configuration item could not be adapted.");

        final var amount = section.getDouble("amount", 0D);

        final var modifiersSection = Objects.requireNonNull(
                section.getConfigurationSection("modifiers"), "Battery modifiers configuration could not be adapted.");
        ...
        for (final var modifierSectionKey : modifiersSection.getKeys(false)) {
            final var upgrade = Objects.requireNonNull(
                    UpgradeEnum.byValue(modifierSectionKey), "Unknown modifier upgrade: " + modifierSectionKey);
