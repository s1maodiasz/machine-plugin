...
public final class MachineItemFactory extends ItemFactory<MachineConfiguration, MachineData> {
    ...
    public MachineItemFactory(@NotNull NamespacedKey key, @NotNull CustomItemProvider provider, @NotNull Gson gson) {
    ...
    public @NotNull ItemStack create(@NotNull MachineConfiguration configuration, @NotNull MachineData data) {
        ...
        final var lore = item.lore().stream()
                .map(line -> apply(line, configuration, data))
                .toList();
        builder.lore(lore);
    ...
    String resolve(@NotNull String id, @NotNull MachineConfiguration cfg, @NotNull MachineData data) {
        ...
        return switch (id) {
            ...
            case "capacity_value" -> NumberFormatter.format(
                    withUpgrade(cfg, MachineUpgradeEnum.CAPACITY, data.levels().get(MachineUpgradeEnum.CAPACITY)));

            case "efficiency_level" -> String.valueOf(levels.getOrDefault(MachineUpgradeEnum.EFFICIENCY, 0));
            case "efficiency_value" -> NumberFormatter.format(withUpgrade(
                    cfg, MachineUpgradeEnum.EFFICIENCY, data.levels().get(MachineUpgradeEnum.EFFICIENCY)));

            case "consumption_level" -> String.valueOf(levels.getOrDefault(MachineUpgradeEnum.CONSUMPTION, 0));
            case "consumption_value" -> NumberFormatter.format(withUpgrade(
                    cfg, MachineUpgradeEnum.CONSUMPTION, data.levels().get(MachineUpgradeEnum.CONSUMPTION)));

            case "speed_level" -> String.valueOf(levels.getOrDefault(MachineUpgradeEnum.SPEED, 0));
            case "speed_value" -> TimeFormatter.format((long)
                    withUpgrade(cfg, MachineUpgradeEnum.SPEED, data.levels().get(MachineUpgradeEnum.SPEED)));
