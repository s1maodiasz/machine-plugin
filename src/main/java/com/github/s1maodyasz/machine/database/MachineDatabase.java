...
public final class MachineDatabase {
  ...
  public MachineDatabase(@NotNull Plugin plugin) {
    ...
    if (url == null || url.isBlank())
      throw new IllegalArgumentException("Missing config: mongo.url");
    ...
    final var collection =
        client.getDatabase(databaseName).getCollection(collectionName, Machine.class);
    ...
    final long timeoutMillis =
        plugin.getConfig().getLong("machine.database.operationTimeoutMillis", 1500L);
  ...
  public @NotNull CompletableFuture<Optional<Machine>> localizeAsync(
